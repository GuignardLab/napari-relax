...
class PluginSignalHub(QObject):
    ...
    colors_changed = Signal(dict)  # Color mapping updates
    ...
    def _auto_connect_widget(self, widget_name: str, widget_instance) -> None:
        ...
        self.cell_selection_changed.connect(widget_instance.handle_selection_change)
        self.colors_changed.connect(widget_instance.handle_color_change)
        self.label_update_requested.connect(widget_instance.handle_label_update)
        
        # Widget-specific connections (keep hasattr for optional methods)
        if widget_name == "manager" and hasattr(widget_instance, 'handle_manager_update'):
            self.lineage_tree_manager_updated.connect(widget_instance.handle_manager_update)
    ...
    def emit_color_change(self, color_mapping: Dict[str, Any]) -> None:
        """Emit signal when colors are updated."""
