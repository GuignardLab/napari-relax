...
def scan_demo_files():
    ...
    configured_files = {config["filename"] for config in DEMO_DATASETS.values()}
...
def show_configured_files(configured_files):
    ...
    for filepath in configured_files:
        ...
        if config['md5'] and config['md5'] != info['md5']:
            print(f"  ⚠️  MD5 mismatch! Update configuration.")
        if config['size_mb'] and abs(config['size_mb'] - info['size_mb']) > 0.01:
            print(f"  ⚠️  Size mismatch! Update configuration.")
...
def propose_configurations(unconfigured_files):
    ...
    for filepath in unconfigured_files:
        ...
        suggested_name = filepath.stem.replace("_demo", "").replace("-", "_").lower()
        
        print(f"\nFile: {info['filename']}")
        print(f"Suggested dataset name: '{suggested_name}'")
        print(f"Proposed configuration to add to _datasets.py:")
        print(f'    "{suggested_name}": {{')
        print(f'        "filename": "{info["filename"]}",')
        print(f'        "url": None,  # Set after uploading to hosting platform')
        print(f'        "md5": "{info["md5"]}",')
        print(f'        "description": "Demo dataset",  # Update with proper description')
...
def main():
    ...
    if configured_files:
        ...
        print(f"Remember to update the 'url' field in _datasets.py after upload.")
