...
def ensure_demo_data(dataset_name: str = "demo") -> Path:
    ...
    if dataset_name not in DEMO_DATASETS:
        raise ValueError(f"Unknown dataset: {dataset_name}. Available: {list(DEMO_DATASETS.keys())}")
    ...
    if filepath.exists():
        if dataset_config["md5"] is None:
            ...
            print(f"File {filepath} exists but has incorrect hash. Re-downloading...")
            filepath.unlink()
    ...
    print(f"Downloading demo dataset '{dataset_name}' ({dataset_config['size_mb']:.1f} MB)...")
    print(f"This is a one-time download that will be cached locally.")
    ...
    if dataset_config["md5"] is not None:
        if not verify_file_integrity(filepath, dataset_config["md5"]):
            filepath.unlink()
            raise RuntimeError(f"Downloaded file for '{dataset_name}' has incorrect hash")
    ...
    cache_info[dataset_name] = {
        ...
        "md5": calculate_md5(filepath) if dataset_config["md5"] is None else dataset_config["md5"]
    }
