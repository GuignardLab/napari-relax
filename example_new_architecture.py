...
def demonstrate_refactored_architecture():
    ...
    from src.napari_relax._layout_utils import SimpleContainer, create_horizontal_container
    ...
    data_manager = LineageTreeDataManager(viewer)
    print("✓ Created LineageTreeDataManager for data operations")
    
    # 3. Test the updated LineageTreeWidgetBase (backward compatibility)
    from src.napari_relax._util_classes.layer_corrector import LineageTreeWidgetBase
    old_style_widget = LineageTreeWidgetBase(viewer)
    ...
    try:
        from src.napari_relax._widgets import LineageTreeAnalysisWidget, CrossEmbryoComparisonWidget
        ...
        embryo_widget = CrossEmbryoComparisonWidget(viewer)
        print("✓ Main plugin widgets created successfully")
        print(f"  - LineageTreeAnalysisWidget with signal hub")
        print(f"  - CrossEmbryoComparisonWidget with signal hub")
    ...
    container = SimpleContainer([button1, button2], horizontal=True)
