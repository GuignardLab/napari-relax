...
For this plugin the [Points Layer] (https://napari.org/stable/howtos/layers/points.html) is the most common and important type of layer.

![viewer](./viewer.png)

## Important Napari Viewer controls panel:

1. **Napari Terminal**: Use the Napari integrated terminal, extremely useful for quick modifications of the dataset (rotation, translation, scaling).

- **3-D viewer**: Toggle 3-D view, essential for navigating 3-D/4D datasets **(Ctrl+Y)**.
- **Grid view**: The grid view is only important if multiple datasets are shown simultaneously on viewer, which overlap **(Ctrl+G)**.
